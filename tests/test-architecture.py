...
def resolve_local_include(relative: str, include: str) -> str | None:
    ...
    owner = ROOT / relative
    candidates = (owner.parent / include, ROOT / include, ROOT / "src" / include)
...
def main() -> None:
    ...
    assert "GVariant" not in code("src/calendar-system.c")
    assert "GVariant" not in code("src/event-store.c")
    assert "g_timeout" not in code("src/system-clock.c")
    assert "GVariant" in read("src/calendar-gvariant-adapter.c")
    assert "GVariant" in read("src/event-gvariant-adapter.c")
    assert "g_timeout_add_full" in read("src/clock-glib-adapter.c")
    ...
    public_event_adapter = read("src/event-store.h")
    ...
    assert "CalendarPlusEventSource" in read("src/event-source.h")
