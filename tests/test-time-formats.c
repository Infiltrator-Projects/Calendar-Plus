...
assert_time_at_location(const gchar *mode,
                        ...
                        const gchar *expected)
{
    g_autofree gchar *located_mode =
        g_strdup_printf("%s@%.4f", mode, latitude);
    const CalendarPlusTimeMode parsed =
        calendar_plus_time_mode_from_string(located_mode);
...
test_tick_boundaries(void)
{
        ...
        3600000);
    {
        ...
        const guint roman_delay =
            calendar_plus_time_delay_to_next_tick(
                roman_mode, USECONDS(946728000), 0, FALSE, 0.0);
        const guint japanese_delay =
            calendar_plus_time_delay_to_next_tick(
                japanese_mode, USECONDS(946728000), 0, FALSE, 0.0);
...
test_historical_calendar_references(void)
{
    ...
    g_assert_cmpstr(mayan, ==, "13.0.0.0.0");
