...
MAP = ROOT / "src/calendar-plus.map"
BASELINE = ROOT / "tests/abi-baseline.txt"
EXPORTS = ROOT / "tests/exported-symbols.txt"
...
def main() -> None:
    ...
    assert baseline.keys() <= exported, (
        "ABI baseline contains symbols missing from exported-symbols.txt: "
        + ", ".join(sorted(baseline.keys() - exported))
    )
    assert exported == version_map.keys(), (
        "exported-symbols.txt and linker version map disagree: "
        f"exports-only={sorted(exported - version_map.keys())}; "
        f"map-only={sorted(version_map.keys() - exported)}"
    )
