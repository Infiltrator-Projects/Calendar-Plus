...
[ "$(unzip -p "$SOURCE" "Calendar-Plus-${VERSION}/shared/infiltratr-common/VERSION" | tr -d '[:space:]')" = "1.14.0" ] || {
    echo "Source ZIP does not vendor the pinned Infiltratr Common 1.14.0 tree" >&2
    exit 1
}
