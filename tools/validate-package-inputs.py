...
def validate_abi_manifest() -> None:
    version_map = (ROOT / "src/calendar-plus.map").read_text(encoding="utf-8")
    allowlist = {
        line.strip()
        for line in (ROOT / "tests/exported-symbols.txt").read_text(
            encoding="utf-8"
        ).splitlines()
        if line.strip()
    }
    ...
    assert mapped == allowlist, (
        f"version-map/export allowlist drift: map={sorted(mapped)}, "
